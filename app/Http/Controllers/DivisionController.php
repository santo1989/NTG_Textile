...
class DivisionController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        $departments = Department::where('division_id', $id);
        $designations = Designation::where('division_id', $id);
        if($companies->count() > 0 || $departments->count() > 0 || $designations->count() > 0){
        $companies->delete();
        $departments->delete();
        $designations->delete();
        $divisions->delete();
        ...
        return redirect()->route('divisions.index')->withMessage('Division and related data of Company Name, Designation Name, Department Name are deleted successfully!');
    }
}
